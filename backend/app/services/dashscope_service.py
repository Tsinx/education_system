...
5) “知识点下不支持添加子级”：若某节点存在 sub_points，该节点应视为“分类节点”；真正“知识点”应为叶子节点（sub_points 为空）
6) 标签规则需明确考虑：固定标签包括“重点、难点、考点、课程思政”，可自定义，多个标签以英文分号 ";" 分隔
7) 知识点分类只允许：事实性、概念性、程序性、元认知。分类节点不填写知识点分类；知识点节点必须填写且仅填写一个分类
8) description 使用课程语境下的定义/作用说明，1-2句。若原文无描述或描述极少（如PPT仅列标题），应基于你的学科知识补充合理的教学定义，使描述具备教学可用性
9) sub_point_descriptions 的 key 必须来自 sub_points，value 为 1 句定义/作用说明。同样允许在原文缺失时基于知识补充
10) 优先忠实原文；若原文存在公式转写损坏、PPT短句不完整、语序破碎等问题，可在不新增知识点的前提下，对 description 与 sub_point_descriptions 做最小必要的格式化修复与语言补全
11) 禁止凭空新增原文中不存在的新知识点、新子知识点或新结论；仅允许在保持知识点实体不变的前提下，补全或优化其描述文本，使其达到可直接用于教案编写的完整度
...
{{
  "points": [
    {{
      ...
      "tags": "重点;难点",
      "knowledge_category": "事实性|概念性|程序性|元认知（分类节点填空字符串）",
...
3) 每个字段与主抽取一致：knowledge_point、sub_points、description、tags、knowledge_category、sub_point_descriptions
4) “知识点下不支持添加子级”：若某节点存在 sub_points，该节点应视为分类节点，不应标记为知识点
5) 标签规则需明确考虑：固定标签包括“重点、难点、考点、课程思政”，可自定义，多个标签以英文分号 ";" 分隔
6) 知识点分类只允许：事实性、概念性、程序性、元认知。分类节点不填写知识点分类；知识点节点必须填写且仅填写一个分类
7) 不输出已有知识点或其同义改写
8) 补充项应尽量忠实原文；若原文表达残缺，可仅在描述字段做最小必要的语言补全，不得引入新的知识点实体
